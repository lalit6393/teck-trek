...
const Timer = () => {
  ...
  const cooldownTime = useRef(
    isCooldown ? new Date().getTime() + cooldownTimer * 1000 + 5000 : startDate
  );
  const navigate = useNavigate();
  const [time, setTime] = useState(timeLeft(cooldownTime.current, isCooldown));
      ...
      .then((res) => {
        ...
        if (res.data.detail?.question || res.data.detail?.time_left == 1) {
          ...
          if (res.data.detail.time_left > 1) {
            ...
            cooldownTime.current =
              new Date().getTime() + res.data.detail.time_left * 1000 + 5000;
            isLoading.current = false;
          }
        }
      })
      ...
      {isLoading.current ? (
        ...
        <div className={styles.timer}>
          ...
          <div className={styles.time}>
            <p>
              {isCooldown
                ? "The next question will be available in"
                : "Let the trek for the wisest begin in"}
            </p>
